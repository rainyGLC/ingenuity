$(function(){
	...
	const cardPage ={
		...
		bind:function(){
			...
			$('.bless-item').on('click', '.bless-close',this.closeItem)

			$('.fixed-nav li').on('click',this.goScroll);//点击右侧导航
			// $(window).on('click').on('scroll',this.windowScroll)//页面滚动
		},
		...
		setCard:function(){
			let background      = ['#f0f5ff','#fdfbe9','#fdfbe9','#f0f5ff'];
			...
			cardList.forEach(function(card,index){
				...
				let tmp = `<div class = "bless-item" style="z-index:${index+1};
		...
		addItem:function(index){
			...
			console.log(value);
			if(value){
				let color     			= ['#fdfbe9','#f0f5ff','#fdfbe9','#f0f5ff'];
				let containerWidth  = $('.bless-card').width();
				let containerHeight = $('.bless-card').height();
				let itemWidth       = 320;
				let itemHeight      = 158;
				...
				let html = `<div class = "bless-item" style="
		...
		closeItem:function(e){
			...
			if(!hasClassActive){
            $('.bless-close.active').removeClass('active');
            $(this).addClass('active');
            $('.bless-item').toggleClass('active');
          }
		...
		toggleNavigatorFixed:function(scrollTop){
			let shouldfixedTop = scrollTop >cardPage.data.headerHeight + cardPage.data.borderHeight;
			console.log(shouldfixedTop);
			let hasfixedTop = $('.mainer-container').hasClass('fixed-top');
			let borderHeight = cardPage.data.borderHeight;
			console.log(hasfixedTop);//false
			if(shouldfixedTop && !hasfixedTop){
				$('.mainer-container').addClass('fixed-top');
			}else if(!shouldfixedTop && hasfixedTop){
				$('.mainer-container').removeClass('fixed-top');
			}
		},
	}
	cardPage.init();
